...
namespace Untils
{
    public class FileHelper
    {
        ...
        public static void CreateFile(string dir, string pagestr, bool append)
        {
            ...
            try
            {
                ...
                if (dir.IndexOf("\\") > -1)
                    CreateDir(dir.Substring(0, dir.LastIndexOf("\\")));
                sw = new StreamWriter(dir, append, System.Text.Encoding.GetEncoding("UTF-8"));
                sw.Write(pagestr);
            }
        ...
        {
            dir1 = MapPath(dir1).Replace("/", "\\");
            dir2 = MapPath(dir1).Replace("/", "\\");
            if (File.Exists(dir1))
                File.Move(dir1, dir2);
        }
        #endregion
        ...
        public static void CreateFile(string filePath, byte[] buffer, bool isAppend)
        {
            try
            {
                if (!isAppend)
        ...
        {
            directoryPath = MapPath(directoryPath);//HttpContext.Current.Server.MapPath(directoryPath);
            if (IsExistDirectory(directoryPath))
            {
                //删除目录中所有的文件
                string[] fileNames = GetFileNames(directoryPath);
        ...
        public static string Search(string fileName, string path = "")
        {
            string rootPath = path == "" ? AppDomain.CurrentDomain.BaseDirectory : path;
            string[] files = System.IO.Directory.GetFiles(rootPath, $"{fileName}.*", System.IO.SearchOption.AllDirectories);
