    ...
    public class ModelHelper<T> where T :new()
    {
        ...
        public static List<T> FillModel(DataTable dt)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                foreach (PropertyInfo propertyInfo in typeof(T).GetProperties())
                {
                    try
                    {
                            object v = dr[propertyInfo.Name] == DBNull.Value ? null : dr[propertyInfo.Name];
                            model.GetType().GetProperty(propertyInfo.Name).SetValue(model, v, null);
                    }
                    catch { continue; }
                }
                modelList.Add(model);
        ...
        public static T FillModel(DataRow dr)
        {
            ...
            foreach (PropertyInfo propertyInfo in typeof(T).GetProperties())
            {
                try
                {
                    if (propertyInfo.Name.ToUpper() != "TRANSNUM")
                    {
                        object v = dr[propertyInfo.Name] == DBNull.Value ? null : dr[propertyInfo.Name];
                        model.GetType().GetProperty(propertyInfo.Name).SetValue(model, v, null);
                    }
